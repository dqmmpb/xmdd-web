 ...
 * =====================================================
 */
$.smVersion = '0.6.1';/* global jQuery:true */
...
(function () {
    ...
    function FastClick(layer, options) {
        ...
        function bind(method, contextT) {
            return function() { return method.apply(contextT, arguments); };
        }
...
(function ($) {
    ...
    $.hidePreloader = function () {
        if($.showPreloader.preloaderModal)
    ...
    $.actions = function (params) {
        ...
        groups.each(function (index, el) {
            ...
            $(el).children().each(function (ind, elem) {
                var buttonIndex = ind;
                var buttonParams = params[groupIndex][buttonIndex];
                var clickTarget;
                if ($(elem).is(buttonSelector)) clickTarget = $(elem);
                // if (toPopover && $(elem).find(buttonSelector).length > 0) clickTarget = $(elem).find(buttonSelector);
    ...
    $(document).on('click', ' .modal-overlay, .popup-overlay, .swipeout-delete, .swipeout-close, .close-popup, .open-popup, .close-picker', handleClicks);
...
(function($) {
    ...
    var getDaysByMonthAndYear = function(month, year) {
        var intD = new Date(year, parseInt(month) + 1 - 1, 1);
        var d = new Date(intD - 1);
  ...
  IScroll.prototype = {
        ...
        _initIndicators: function() {
            ...
            function _indicatorsMap(fn) {
                for (var j = that.indicators.length; j--; ) {
                    fn.call(that.indicators[j]);
                }
            }
...
(function ($) {
    ...
    var showTab = function (tab, tabLink, force) {
        ...
        if (tabLink.length === 0) return false;
...
(function($) {
    ...
    $.initPullToRefresh = function(pageContainer) {
        ...
        function handleTouchStart(e) {
            if (isTouched) {
                if ($.device.android) {
                    if ('targetTouches' in e && e.originalEvent.targetTouches.length > 1) return;
                } else return;
            }
            isMoved = false;
            isTouched = true;
...
(function($){
  ...
  $.initSwipeout = function (swipeoutEl) {
    ...
    var swipeoutActionsNoFold = true;
    var swipeoutNoFollow = true;
    ...
    function handleTouchMove(e) {
      ...
      if (!isMoved) {
        ...
        noFoldLeft = actionsLeft.hasClass('swipeout-actions-no-fold') || swipeoutActionsNoFold;
        noFoldRight = actionsRight.hasClass('swipeout-actions-no-fold') || swipeoutActionsNoFold;
  ...
  $.swipeoutOpen = function (el, dir, callback) {
    ...
    var noFold = swipeOutActions.hasClass('swipeout-actions-no-fold') || swipeoutActionsNoFold;
...
(function($) {
    ...
    Router.prototype._loadDocument = function(url, callback) {
        ...
        this.xhr = $.ajax({
            ...
            success: $.proxy(function(data, status, xhr) {
                ...
                $doc.append(data);
                if(callback.success)
                  callback.success.call(null, $doc, status, xhr);
            }, this),
            error: function(xhr, status, err) {
                if(callback.error)
                  callback.error.call(null, xhr, status, err);
                self.dispatch('pageLoadError');
            },
            complete: function(xhr, status) {
                if(callback.complete)
                  callback.complete.call(null, xhr, status);
    ...
    $(function() {
        ...
        if (!$pages.length) {
            ...
            if(logFn)
              logFn('Disable router function because of no .page elements');
            return;
        }
